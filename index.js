...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:𝐏𝐈𝐑𝐄𝐒\n' 
            + 'ORG: 𝐏𝐈𝐑𝐄𝐒 BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=556996103915:+55 69 96103915\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["556996103915@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oi @${num.split('@')[0]}\ʙᴇᴍ ᴠɪɴᴅᴏ ᴀ ᴇsᴛᴇ ɢʀᴜᴘᴏ, ᴍᴇɴᴏs ɴᴏᴛᴀ ғᴀᴋᴇ ᴇ ᴢᴇ ᴛʀᴀᴠɪɴʜᴀ  *${mdata.subject}* 
┏━━━━━━━━━━━━━━━
┃────「 *_ᴀᴘʀᴇsᴇɴᴛᴀÇÃᴏ_* 」─────
┃━━━━━━━━━━━━━━━
┠⊷️ *ɴᴏᴍᴇ* :
...
┗━━━━━━━━━━━━━━━`
				...
				teks = `finalmente a carga do grupo é reduzida 𝟭, tchau tchau🥳 @${num.split('@')[0]} Seus serviços estarão na cova profunda²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!tex) return client.sendMessage(from, 'INSIRA O URL / TEXTO PARA FAZER UM QR ', text, {quoted: mek})
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                if (isGroup) {
                    ...
                    addATM(sender)
                    addLevelingId(sender)
                    console.log(color('[REGISTER]'), color(time, 'yellow'), 'Nome:', color(namaUser, 'cyan'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
                }
				break
				//terhambar 
				case 'quotemaker':
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'PROPRIETÁRIO Pires :v',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				const dare =['Envie uma mensagem para o seu ex dizendo "Eu ainda gosto de você", "ligue crush/pacar sekarang dan ss ke pemain','pap ke salah satu anggota grup','Bilang "KAMU CANTIK BANGET NGGAK BOHONG" ke cowo','ss recent call whatsapp','drop emot "🦄💨" setiap ngetik di gc/pc selama 1 hari','kirim voice note bilang can i call u baby?','drop kutipan lagu/quote, terus tag member yang cocok buat kutipan itu','pake foto sule sampe 3 hari','ketik pake bahasa daerah 24 jam','ganti nama menjadi "gue anak lucinta luna" selama 5 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kautsar','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','teriak pake kata kasar sambil vn trus kirim kesini','teriak " anjimm gabutt anjimmm " di depan rumah mu','ganti nama jadi " BOWO " selama 24 jam','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll']
